...
#'
#' @description hier evtl wie das geht und welche threshold func ich nehm
#' @export
#' @param x.train  vector of independent variables used for training
         ...
         pars=list(var=1, inv.scale=2, gamma=2, noise=.1, kernel="gamma.exp"), ...)
{
  ...
  lvgpr.obj <- new.lvgpr.obj(x.train=x.train, y.train=y.train, x.new=x.new,
                             pars=pars)
  ...
  obj <- list(y.predict=pred.posterior$y.predict,
         ...
         pars=list(var=1, inv.scale=2, gamma=2, noise=.1, kernel="gamma.exp"), ...)
{
    ...
    lvgpc.obj <- new.lvgpc.obj(x.train=x.train, c.train=c.train, x.new=x.new,
                               pars=pars)
    # predict the posterior means, covariance,
