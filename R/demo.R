...
function()
{
  ...
  # create empty plot
  graphics::plot(type="n", x, y, col=1, main = "Prior GP",ylim=c(-3,3), xlim=c(-5,5))
...
function()
{
    ...
    # create testing data
    x.new <- base::sort(c(stats::rnorm(20, -5), stats::rnorm(20, 5), stats::rnorm(20)))
    ...
    for (i in 1:3)
    {
      pred <- lvgpc(x.train=x.train, c.train=c.train, x.new=x.new, pars=params)
      c <- pred$c.predict
